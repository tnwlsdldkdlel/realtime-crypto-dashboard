...
- [x] Header 네비게이션 링크 활성화 (탭 기능 구현 완료)
...
#### 2.3 목록 가상화 구현
- [x] TanStack Virtual 통합
- [x] 가상화된 리스트 컴포넌트 구현
- [x] 성능 테스트 및 최적화

#### 2.4 정렬 및 필터링 기능
- [x] 정렬 기능 구현 (가격, 변동률, 거래량 등)
- [x] 필터링 기능 구현 (심볼 검색)
- [x] 정렬/필터 상태 관리
...
#### 3.1 가격 변경 하이라이트
- [x] 가격 변경 감지 로직 구현 (이전 가격 추적 및 비교)
- [x] 300ms 소프트 하이라이트 애니메이션 구현 (배경색 트랜지션)
- [x] 색상 트랜지션 (상승: 녹색, 하락: 빨간색)

#### 3.2 시각적 계층 구조
- [x] 현재가 및 변동률 강조 스타일링 (폰트 크기 및 굵기 조정)
- [x] 폰트 크기 및 색상 계층 구조 적용 (현재가: text-lg, 변동률: text-base)
- [x] 반응형 디자인 고려 (기존 반응형 레이아웃 유지)

#### 3.3 애니메이션 스로틀링
- [x] 100ms 단위 애니메이션 제한 구현 (lastHighlightTimeRef로 제어)
- [x] `requestAnimationFrame` 기반 애니메이션 최적화 (배치 처리)
...
#### 4.1 즐겨찾기 UI 구현
- [x] 즐겨찾기 버튼/아이콘 컴포넌트
- [x] 즐겨찾기 목록 표시 컴포넌트 (개수 표시)
- [x] 즐겨찾기 토글 기능

#### 4.2 WebSocket 구독 자동 조정
- [x] 즐겨찾기 변경 시 WebSocket 재구독 로직
- [x] 디바운스된 재연결 구현 (300ms)
- [x] 구독 상태 관리 및 UI 피드백

#### 4.3 즐겨찾기 탭 페이지 구현
- [x] `/favorites` 라우트 생성
- [x] 즐겨찾기 전용 페이지 컴포넌트 생성
- [x] 즐겨찾기된 코인만 표시하는 CoinListClient 재사용 (`favoritesOnly` prop 추가)
- [x] WebSocket은 즐겨찾기 코인만 구독
- [x] Header 네비게이션 링크 활성화 (Next.js Link 사용)
- [x] 즐겨찾기가 없을 때 안내 메시지 표시
...
- [x] 차트 컨테이너 컴포넌트 생성 (`CandlestickChart` 컴포넌트)
- [x] 기본 캔들스틱 차트 렌더링

#### 5.2 과거 데이터 페칭 및 표시
- [x] 코인 선택 시 과거 OHLCV 데이터 페칭 (ChartClient에서 구현)
- [x] 차트에 과거 데이터 표시
- [x] 로딩 상태 처리

#### 5.3 실시간 Kline 스트림 통합
- [x] WebSocket Kline 스트림 구독 (BinanceWebSocketClient 통합)
- [x] 실시간 데이터와 과거 데이터 병합 로직 (`updateChartDataWithRealtimeKline`)
- [x] `openTime` 기준 갱신/추가 결정 (기존 캔들 업데이트 또는 새 캔들 추가)

#### 5.4 데이터 간격(Gap) 처리
- [x] 데이터 간격 감지 로직 (`detectAndFillGaps` 함수)
- [x] REST API를 통한 간격 메우기 (간격 발견 시 자동으로 데이터 채움)
- [x] 데이터 정합성 보장 (시간 순서 정렬 및 중복 제거)

#### 5.5 차트 인터랙션
- [x] 차트 상세 페이지/모달 구현 (현재 페이지에서 충분히 상세 정보 제공)
- [x] 줌, 패닝 기능 활용 (마우스 휠 줌, 드래그 패닝, 터치 제스처 지원)
- [x] 시간대 선택 기능 (1m, 5m, 15m, 1h, 4h, 1d 선택 가능)

#### 5.6 차트 탭 페이지 구현
- [x] `/chart` 라우트 생성 (기본 구조)
- [x] 차트 전용 페이지 컴포넌트 생성 (`ChartClient` 컴포넌트)
- [x] Header 네비게이션 링크 활성화
- [x] 코인 선택 UI 구현 (드롭다운으로 코인 선택)
- [x] 선택된 코인의 차트 표시
...
| **Phase 4** | 즐겨찾기 기능 구현 | 1일 (탭 페이지 포함 시 1-2일) |
| **Phase 5** | 인터랙티브 차트 구현 | 3-4일 (탭 페이지 포함 시 4-5일) |
